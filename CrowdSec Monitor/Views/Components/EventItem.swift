    ...
    var body: some View {
        ...
        Button {
            ...
            VStack(alignment: .leading) {
                ...
                if let targetFqdn = targetFqdn {
                    HStack {
                        Spacer()
                        Text(targetFqdn.value)
                            .fontWeight(.semibold)
                            .font(.system(size: 16))
                    }
                    Spacer()
                ...
                // HTTP Events
                if let httpVerb = httpVerb, let httpPath = httpPath, let httpStatus = httpStatus, let httpUserAgent = httpUserAgent {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text(verbatim: isValidUserAgent(httpUserAgent.value))
                        }
                            .font(.system(size: 12))
                            .fontWeight(.medium)
                            .foregroundStyle(Color.gray)
                            .fontDesign(.monospaced)
                        if let datasourcePath = datasourcePath {
                            HStack(spacing: 8) {
                                Image(systemName: "doc.text")
                                    .foregroundStyle(Color.gray)
                                    .font(.system(size: 12))
                                Text(verbatim: datasourcePath.value)
                                    .font(.system(size: 11))
                                    .fontWeight(.medium)
                                    .foregroundStyle(Color.gray)
                                    .fontDesign(.monospaced)
                ...
                // SSH Events
                else if let logType = logType, logType.value.contains("ssh"), let service = service {
                    VStack(alignment: .leading, spacing: 12) {
                        HStack(spacing: 12) {
                            Text(service.value.uppercased())
                                .fontWeight(.semibold)
                                .font(.system(size: 14))
                                .padding(.horizontal, 8)
                                .padding(.vertical, 4)
                ...
                else if let logType = logType, let service = service {
                    VStack(alignment: .leading, spacing: 12) {
                        HStack(spacing: 12) {
                            Text(service.value.uppercased())
                                .fontWeight(.semibold)
